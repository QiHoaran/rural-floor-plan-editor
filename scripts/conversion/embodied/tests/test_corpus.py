...
import pytest
from fixtures import _two_record_fixture, raw_rectangle


def test_cleaned_adapter_preserves_null_semantics_and_unsupported_geometry():
    from embodied import corpus
...
def test_corpus_publication_audit_and_repeatability(tmp_path):
    from embodied import corpus

    source = _two_record_fixture(tmp_path)
    first, second = tmp_path / "first", tmp_path / "second"
    summary = corpus.build_corpus(source, first)
    assert summary["input_building_count"] == 2
    assert summary["valid_building_count"] == 2
    assert summary["quarantined_building_count"] == 0
    assert corpus.audit_corpus(first, source)["status"] == "valid"
    corpus.build_corpus(source, second)
...
def test_source_integrity_failure_never_publishes(tmp_path):
    ...
    with pytest.raises(ValueError, match="HASH_MISMATCH"):
        corpus.build_corpus(source, tmp_path / "out")
...
def test_codec_failure_blocks_publication(tmp_path, monkeypatch, reason):
    ...
    source = _two_record_fixture(tmp_path)
    original = corpus.build_artifacts
    ...
    monkeypatch.setattr(corpus, "build_artifacts", injected_failure)
