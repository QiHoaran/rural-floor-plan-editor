"""Batch CLI for cleaned → CAD training data."""
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="conversion-cad",
        description="Convert cleaned rural floor plans to CAD training data.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    """Run the CAD converter and emit exactly one machine-readable result."""
    ...
    if input_root is None or output_root is None:
        ...
        output_root = output_root or (
            repository_root / "data" / "rural_data" / "model_ready" / "cad"
        )
    try:
