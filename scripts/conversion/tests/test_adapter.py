...
sys.path.insert(0, str(Path(__file__).resolve().parents[1] / "embodied" / "tests"))
import adapter
from fixtures import raw_rectangle
from conversion_cad.cad import build_cad_primitives
from conversion_graph.graph import build_graph_record
from conversion_image.image import render_training_masks
...
class AdapterTests(unittest.TestCase):
    ...
    def test_quarantine_is_distinct_from_programming_errors(self):
        ...
        events = []
        adapter.convert(self.request(["embodied", "graph"]), events.append)
        ...
        events = []
        with patch("embodied.pipeline.build_artifacts", side_effect=ValueError("unclassified codec bug")):
            adapter.convert(self.request(["embodied"]), events.append)
    ...
    def test_codec_invariants_are_failures_even_if_reference_quarantines_them(self):
        for code in ("TOKEN_GRAMMAR_ERROR", "FLOORPLAN_ROUNDTRIP_MISMATCH", "NON_DETERMINISTIC_REENCODE"):
            events = []
            with patch("embodied.pipeline.build_artifacts", return_value={"status": "quarantined", "reason_code": code, "reason": f"{code}: test"}):
                adapter.convert(self.request(["embodied"]), events.append)
            self.assertEqual(events[0]["status"], "failed")
