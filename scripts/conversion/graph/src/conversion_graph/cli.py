"""Batch CLI for cleaned → Graph training data."""
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="conversion-graph",
        description="Convert cleaned rural floor plans to graph training data.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    """Run the graph converter and emit exactly one machine-readable result."""
    ...
    if input_root is None or output_root is None:
        ...
        output_root = output_root or (
            repository_root / "data" / "rural_data" / "model_ready" / "graph"
        )
    try:
