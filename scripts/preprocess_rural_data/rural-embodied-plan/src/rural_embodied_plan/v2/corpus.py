"""Verified, no-overwrite v2 corpus generation and source-backed read-back audit."""
...
from rural_embodied_plan.v2.pipeline import build_v2_artifacts, json_value
...
def summarize(records: list[dict[str, Any]], config: V2Config) -> dict[str, Any]:
...
def audit_v2_corpus(
    ...
    config = V2Config.model_validate(read(output / "robot_config.json"))
    ...
    for index, (record, original) in enumerate(zip(records, source, strict=True), 1):
        ...
        with tempfile.TemporaryDirectory(prefix="v2-readback-") as temp:
...
def build_v2_corpus(
    ...
    audit = audit_v2_corpus(staging, input_root, progress)
    write(staging / "dataset_audit.json", audit)
    lines = [
        "# Embodied v2 corpus generation + audit",
        "",
