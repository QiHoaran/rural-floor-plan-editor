...
from rural_embodied_plan.v2.floorplan import CanonicalFloorplan, canonicalize_floorplan
from rural_embodied_plan.v2.floorplan_encoder import Encoder, encode_floorplan
from rural_embodied_plan.v2.planner import shift
from rural_embodied_plan.v2.roundtrip_validator import validate_roundtrip
from rural_embodied_plan.v2.schemas import BehaviorTokens, TimedTrajectory
...
def build_v2_artifacts(
    document: BuildingDocument | dict[str, Any], output: Path, config: V2Config
) -> dict[str, Any]:
    if output.exists():
    ...
    schemas = {
        "robot_config.json": V2Config.model_json_schema(),
    ...
    staging = Path(tempfile.mkdtemp(prefix=f".{output.name}-v2-", dir=output.parent))
