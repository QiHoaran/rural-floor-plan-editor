...
def test_terminal_heading_included_and_path_unique() -> None:
    ...
    from rural_embodied_plan.v2.planner import plan_path

    polygon = ((0, 0), (5000, 0), (5000, 4000), (0, 4000))
    path = plan_path(polygon, (F(1000), F(1000)), (F(3000), F(3000)), 1, 0, V2Config())
...
def test_nonconvex_path_and_impossible_anchor() -> None:
    ...
    path = plan_path(polygon, (F(1000), F(5000)), (F(5000), F(5000)), 2, 0, V2Config())
    assert min(p[1] for p in path) <= 1750
    with pytest.raises(ValueError, match="PATH_INVALID"):
        plan_path(polygon, (F(0), F(0)), (F(5000), F(5000)), 2, 0, V2Config())
