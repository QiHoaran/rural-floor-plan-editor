...
from rural_embodied_plan.v2.exact import (
    duration_tokens,
    rational_tokens,
    string_tokens,
    uint_tokens,
...
def test_decoder_first_manual_exact_room() -> None:
    ...
    f = decode_behavior_tokens(manual_room_tokens(), V2Config())
...
def test_decoder_rejects_future_room_and_missing_reset() -> None:
    ...
    with pytest.raises(ValueError, match="TOKEN_GRAMMAR_ERROR"):
        decode_behavior_tokens(tokens, V2Config())
    tokens = manual_room_tokens()
...
def test_encode_decode_reencode_exact() -> None:
    ...
    reconstructed = decode_behavior_tokens(tokens, V2Config())
    assert reconstructed == f
    assert encode_floorplan(reconstructed, V2Config()) == tokens
...
def test_missing_exit_is_not_a_valid_stopped_episode() -> None:
    ...
    del t[start:end]
    with pytest.raises(ValueError, match="TOKEN_GRAMMAR_ERROR"):
        decode_behavior_tokens(t, V2Config())
...
def test_hand_authored_fixture_is_a_canonical_token_sequence() -> None:
    ...
    tokens = manual_room_tokens()
    assert encode_floorplan(decode_behavior_tokens(tokens, V2Config()), V2Config()) == tokens
...
def test_source_id_renaming_does_not_change_stream() -> None:
    ...
    reference = encode_floorplan(
        canonicalize_floorplan(BuildingDocument.model_validate(raw)), V2Config()
    )
