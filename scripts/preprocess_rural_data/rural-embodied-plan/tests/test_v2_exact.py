...
def test_fraction_and_utf8_are_lossless() -> None:
    from rural_embodied_plan.v2.exact import Reader, rational_tokens, string_tokens
...
def test_noncanonical_integer_rejected(tokens: list[str]) -> None:
    from rural_embodied_plan.v2.exact import Reader
...
def test_exact_time_and_bin_checked() -> None:
    ...
    from rural_embodied_plan.v2.exact import Reader, duration_tokens

    config = V2Config(linear_speed_mm_s=3000)
...
def test_config_strict_and_positive() -> None:
    ...
    with pytest.raises(ValueError):
        V2Config(linear_speed_mm_s=0)
    with pytest.raises(ValueError):
        V2Config(duration_bins_ms=(0, 100, 100))
