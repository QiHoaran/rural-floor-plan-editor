...
class Converter:
    ...
    def availability(self) -> dict[str, Any]:
        try:
            ...
            return {"available": False, "message": f"Conversion environment unavailable: {exc}. Run uv sync --all-packages --all-groups --locked in scripts/preprocess_rural_data."}
...
def graph(context: Context, output: Path) -> dict[str, Any]:
    from rural_data_prep import multimodal as m
    value = m.build_graph_record(context.cleaned.canonical, context.cleaned.training)
    schema = m.graph_schema_document()
    m.validate_json_schema(value, schema)
    m._validate_graph_integrity(value)
    output.mkdir()
    write_json(output / "graph.json", value)
    write_json(output / "graph.schema.json", schema)
    write_json(output / "vocabulary.json", m.multimodal_vocabulary())
...
def image(context: Context, output: Path) -> dict[str, Any]:
    ...
    rendered = m.render_training_masks(context.cleaned.training)
    schema = m.image_schema_document()
    m.validate_json_schema(rendered.stats, schema)
    m._validate_image_integrity(rendered)
    output.mkdir()
    for name, mask in (("semantic", rendered.semantic), ("instance", rendered.instance)):
        ...
        with Image.open(path) as loaded:
            if loaded.size != (256, 256) or m._histogram(loaded) != rendered.stats[f"{name}_histogram"]:
...
def cad(context: Context, output: Path) -> dict[str, Any]:
    from rural_data_prep import multimodal as m
    value = m.build_cad_primitives(context.cleaned.canonical)
    schema = m.cad_schema_document()
    m.validate_json_schema(value, schema)
    m._validate_cad_integrity(value)
    output.mkdir()
    write_json(output / "primitives.json", value)
    write_json(output / "cad.schema.json", schema)
    write_json(output / "vocabulary.json", m.multimodal_vocabulary())
    m.write_dxf(value, output / "building.dxf")
...
def embodied(context: Context, output: Path) -> dict[str, Any]:
    ...
    config = V2Config()
    report = build_v2_artifacts(context.document, output, config)
...
REGISTRY = {item.id: item for item in (
    Converter("graph", "Graph", "Graph", "1.0.0", ("rural_data_prep.multimodal",), graph, True),
    Converter("image", "Image", "Image", "1.0.0", ("rural_data_prep.multimodal",), image, True),
    Converter("cad", "CAD", "CAD", "1.0.0", ("rural_data_prep.multimodal", "ezdxf"), cad, True),
    Converter("embodied_v2", "Embodied v2", "Embodied", "2.0.0", ("rural_embodied_plan.v2.pipeline",), embodied),
)}
