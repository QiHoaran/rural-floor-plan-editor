...
from rural_data_prep.pipeline import _corpus_hash, stable_json_bytes
from rural_data_prep.records import build_records
from tests.test_records import sample_document, source_for

build_graph_record = getattr(multimodal, "build_graph_record", None)
load_cleaned_corpus = getattr(multimodal, "load_cleaned_corpus", None)
graph_schema_document = getattr(multimodal, "graph_schema_document", None)
multimodal_vocabulary = getattr(multimodal, "multimodal_vocabulary", None)
...
class GraphConversionTest(unittest.TestCase):
    ...
    def test_publishes_versioned_graph_schema_and_vocabulary(self) -> None:
        ...
        multimodal.validate_json_schema(graph, schema)
        invalid = dict(graph)
        invalid.pop("edges")
        with self.assertRaisesRegex(ValueError, "edges is required"):
            multimodal.validate_json_schema(invalid, schema)
        ...
        with self.assertRaisesRegex(ValueError, "too many items"):
            multimodal.validate_json_schema(malformed_node, schema)
