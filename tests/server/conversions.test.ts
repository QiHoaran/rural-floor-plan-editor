...
it('exposes async job APIs, structured input errors and opening the registered output root',async()=>{
  ...
  expect(formats.body.formats).toContainEqual(expect.objectContaining({id:'housegan',label:'HouseGAN',directory:'HouseGAN',usesClean:true}));
  expect(formats.body.formats).toContainEqual(expect.objectContaining({id:'embodied',usesClean:false}));
