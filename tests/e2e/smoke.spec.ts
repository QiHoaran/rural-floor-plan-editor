...
test.describe('乡村住宅矢量编辑器 E2E 冒烟测试', () => {
  ...
  test('编辑器可完成审核、完成和当前 revision 导出流程', async ({
    ...
    const before = await (await request.get('/api/projects/' + buildingId)).json();
    await page.getByLabel('村号（rural）').click();
    await expect(page.getByRole('dialog', { name: '项目已完成' })).toBeVisible();
    await page.screenshot({ path: 'tests/test-results/completed-readonly-dialog.png' });
    await page.getByRole('button', { name: '取消', exact: true }).click();
    await page.getByLabel('显示点', { exact: true }).uncheck();
    await page.getByRole('button', { name: '外墙 (W1)', exact: true }).click();
    await expect(page.getByRole('dialog', { name: '项目已完成' })).toBeVisible();
    const after = await (await request.get('/api/projects/' + buildingId)).json();
    expect(after.document).toEqual(before.document);
    await page.getByRole('button', { name: '重新打开并编辑', exact: true }).click();
    await expect(page.getByRole('dialog')).toHaveCount(0);
    await page.getByLabel('村号（rural）').fill('editable_again');
    await expect(page.getByLabel('村号（rural）')).toHaveValue('editable_again');
  });
});
